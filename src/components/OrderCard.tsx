...
export default function OrderCard({ order, draggable, onDragStart, onDragEnd }: OrderCardProps) {
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-2">
        <h3 className="text-sm font-semibold leading-snug text-slate-100">{order.title}</h3>
        {draggable && (
          <GripVertical
            size={16}
      ...
      <div className="mt-4 flex items-center justify-between">
        <span className="text-base font-bold text-slate-100">{formatCurrency(order.price)}</span>
        <span className={`rounded-full px-2.5 py-1 text-[11px] font-medium ${meta.badge}`}>
          {meta.label}
        </span>
      </div>
    </article>
